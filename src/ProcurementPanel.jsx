...
export default function ProcurementPanel({ p, update }) {
  ...
  const assign = (key, supplier) => update(["procurement", "assignments", key], supplier);
  ...
  return <section className="card" style={{ marginTop: 20 }}>
    <div className="catalogue-title-row">
      <div>
        ...
        <p className="hint">{it
          ? "Anteprima interna generata dai prodotti selezionati nel progetto e dai costi aggiuntivi. Nessun ordine viene inviato automaticamente."
          : "Internal preview generated from products selected in the project and additional project costs. No order is sent automatically."}</p>
      </div>
    </div>
    {!groups.length ? <p className="hint">{it ? "Nessuna voce di acquisto rilevata nel progetto attivo." : "No procurement items detected in the active project."}</p> : groups.map((group) => <div key={group.supplier} style={{ border: "1px solid var(--border, #d8e0ea)", borderRadius: 10, padding: 14, marginTop: 14 }}>
      ...
      <div className="table-scroll"><table><thead><tr>
        <th>{it ? "Fonte" : "Source"}</th><th>Brand</th><th>{it ? "Prodotto / lavoro" : "Product / work"}</th><th>SKU</th><th>{it ? "Quantità" : "Quantity"}</th><th>{it ? "Unità" : "Unit"}</th><th>{it ? "Costo unitario" : "Unit cost"}</th><th>{it ? "Totale" : "Total"}</th><th>{it ? "Fornitore progetto" : "Project supplier"}</th>
      </tr></thead><tbody>{group.items.map((item) => <tr key={item.key}>
        <td>{item.source}</td><td>{item.brand || "—"}</td><td>{item.description}</td><td>{item.supplierSku || "—"}</td><td>{item.quantity}</td><td>{item.unit}</td><td>{money.format(item.unitCost)}</td><td>{money.format(item.totalCost)}</td>
        <td><input value={p.procurement?.assignments?.[item.key] ?? item.supplier ?? ""} placeholder={it ? "Assegna fornitore" : "Assign supplier"} onChange={(e) => assign(item.key, e.target.value)} /></td>
      </tr>)}</tbody></table></div>
    </div>)}
  </section>;
}
