...
export function generateCustomerPdf(project, result) {
  ...
  autoTable(doc, {
    ...
    body: (result.customerCashFlowRows || result.cashFlowRows).map((row) => [row.year, money(row.customerGrossBenefit ?? row.grossBenefit), commercial.opexIncludedInPayment ? commercial.includedLabel : money2(row.customerGrossServiceOpex ?? row.opex), money2(row.customerGrossPayment ?? row.payment), money(row.customerNetCashFlow ?? row.netCashFlow), money(row.customerCumulative ?? row.cumulative)]),
    headStyles: { fillColor: [15, 118, 110] },
