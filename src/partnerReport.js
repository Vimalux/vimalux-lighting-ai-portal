...
import { adaptiveDimmingPartnerOptions, cmsPartnerOptions, partnerProjectRows, partnerTotals } from "./partners.js";
...
export function generatePartnerPdf(partner, projects, language = "it", currency = "EUR") {
  ...
  const isCmsPartner = cmsPartnerOptions(projects).includes(normalized);
  const isAdaptivePartner = adaptiveDimmingPartnerOptions(projects).includes(normalized);
  const roleLabel = isCmsPartner ? "CMS / Lighting Control" : isAdaptivePartner ? "Adaptive Dimming" : "Partner";
  const label = normalized === "VIMALUX" ? "VIMALUX" : `${partner} · ${roleLabel}`;
  ...
  const detailBody = isCmsPartner
    ? rows.map(row=>[row.municipality,row.project,formatProbabilityPoints(row.probability,language),money(row.pipelineTcv),money(row.weightedTcv),money(row.annualRevenue),row.contractYears,money(row.totalContractValue)])
