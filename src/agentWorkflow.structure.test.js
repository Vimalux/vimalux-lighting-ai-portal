...
test("shared workflow includes project additional costs before pricing and assumptions", () => {
  assert.match(appSource, /const workflow = \[[\s\S]*\["solution", "solution"\][\s\S]*\["additionalCosts", "additionalCosts"\][\s\S]*\["pricing", "pricing"\][\s\S]*\["assumptions", "assumptions"\]/);
...
test("additional-cost view is shared while internal pricing remains admin-only", () => {
  assert.match(appSource, /view === "additionalCosts" && <AdditionalCostsCard p=\{project\} update=\{update\} mode=\{isAgent \? "agent" : "admin"\} \/>/);
...
test("agent additional-cost card accepts supplier cost without exposing sales prices", () => {
  assert.match(cardSource, /const isAgent = mode === "agent"/);
  assert.match(cardSource, /showSalesPrices = !isAgent/);
  assert.match(cardSource, /"Costo unitario"/);
  assert.match(cardSource, /change\(index, "unitCost"/);
  assert.match(cardSource, /showSalesPrices && <th>\{it \? "Prezzo unitario"/);
  assert.match(cardSource, /showSalesPrices && <th>\{it \? "Prezzo totale"/);
  assert.doesNotMatch(cardSource, /"Prezzo progetto"/);
  assert.match(cardSource, /isAgent \? \([\s\S]*totals\.capexCost[\s\S]*totals\.annualOpexCost/);
  assert.match(cardSource, /\+ Aggiungi costo/);
});

test("agent update path is explicitly accepted and sanitized", () => {
  assert.match(appSource, /if \(isAgent && path\[0\] === "additionalCosts"\) \{[\s\S]*sanitizeAgentAdditionalCosts\(project\.additionalCosts, value\)/);
});
