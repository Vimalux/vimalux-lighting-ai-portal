...
export default function HybridSummary({ p, update }) {
  ...
  return <section className="card" style={{ marginTop: 20 }}>
    ...
    <div className="table-scroll"><table><thead><tr>
      <th>{it ? "Prodotto" : "Product"}</th><th>{it ? "Qtà" : "Qty"}</th><th>PV Wp</th><th>{it ? "Batteria" : "Battery"}</th><th>{it ? "Modo solare" : "Solar mode"}</th><th>{it ? "Autonomia" : "Autonomy"}</th><th>{it ? "Peso" : "Weight"}</th><th>{it ? "Controllo peso" : "Weight check"}</th><th>{it ? "PV annuo" : "Annual PV"}</th><th>{it ? "Solare utile" : "Usable solar"}</th>
