...
export default function CrmOpportunity({ projects, active, update, money, onImport, onManual, setView, currentUser, getLinkedBusinessCaseId, createOrOpenBusinessCase, onBusinessCaseLinked }) {
  ...
  return <>
    ...
    <Card title={`Opportunity · ${canonical.opportunity.projectName}`} className="opportunity-detail">
      ...
      <div className="opportunity-section"><h3>C. Preliminary Project Data</h3><div className="detail-grid"><span>Luminaires<strong>{canonical.assumptions.totalLuminaires}</strong></span><span>Existing technology<strong>{canonical.assumptions.existingTechnology || "—"}</strong></span><span>Average watt<strong>{canonical.assumptions.averageExistingWatt.toFixed(1)} W</strong></span><span>Annual hours<strong>{canonical.assumptions.annualOperatingHours}</strong></span><span>Energy price<strong>{canonical.assumptions.energyPrice}</strong></span><span>Dimming<strong>{canonical.assumptions.existingDimmingProfile} · {canonical.assumptions.existingDimmingPct}%</strong></span><span>Smart / CMS / Adaptive Dimming<strong>{canonical.assumptions.smartLightingEnabled ? "Yes" : "No"} / {canonical.assumptions.cmsEnabled ? "Yes" : "No"} / {canonical.assumptions.powerAidEnabled ? "Yes" : "No"}</strong></span></div></div>
